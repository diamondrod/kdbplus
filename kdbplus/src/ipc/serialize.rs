...
fn serialize_q(obj: &K, stream: &mut Vec<u8>){
  match obj.0.qtype{
    ...
    qtype::INT_ATOM | qtype::MONTH_ATOM | qtype::DATE_ATOM | qtype::MINUTE_ATOM | qtype::SECOND_ATOM | qtype::TIME_ATOM => serialize_int(obj, stream),
    qtype::LONG_ATOM | qtype::TIMESTAMP_ATOM | qtype::TIMESPAN_ATOM => serialize_long(obj, stream),
    ...
    qtype::INT_LIST | qtype::MONTH_LIST | qtype::DATE_LIST | qtype::MINUTE_LIST | qtype::SECOND_LIST | qtype::TIME_LIST => serialize_int_list(obj, stream),
    qtype::LONG_LIST | qtype::TIMESTAMP_LIST | qtype::TIMESPAN_LIST => serialize_long_list(obj, stream),
...
fn serialize_guid_list(list: &K, stream: &mut Vec<u8>){
  ...
  vector.iter().for_each(|element| stream.extend_from_slice(element));
}
