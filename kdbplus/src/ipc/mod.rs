...
//!
//! *Notes:*
//! - For acceptor, a file path of a credential file must be set on `KDBPLUS_ACCOUNT_FILE`. Credentials must be written in each line with the format
//!  `"user:sha256_encoded_password"`. For example, user `reluctant` and password `"slowday"` is written as
//!  `reluctant:d03f5cc1cdb11a77410ee34e26ca1102e67a893c`. In q, the encoded password can be generated by `.Q.sha1 "slowday"`.
//! - For TS client, you need to set two environmental variables `KX_SSL_CERT_FILE` and `KX_SSL_KEY_FILE` to make kdb+ to
//!  work as a TLS server. For details, see [the KX website](https://code.kx.com/q/kb/ssl/).
//! - For TLS acceptor, user needs to set a path to a pkcs12 file to an environmental variable `KDBPLUS_TLS_KEY_FILE` and a
//! password of the file to `KDBPLUS_TLS_KEY_FILE_SECRET`. Then `.kdbplus.close_tls_connection_` will be defined on kdb+ side
//! by asynchronous call from the server side. This function is necessary to close the handle from the server side without
//! crashing server side application.
//! - For Unix domain socket acceptor, user needs to set a directory where a socket file is created to an environmental variable `QUDSPATH`.
...
//! }
//! ```
//! Suppose the credential file set on `KDBPLUS_ACCOUNT_FILE` is like this;
//! ```bash
//! mattew:431364b6450fc47ccdbf6a2205dfdb1baeb79412
//! reluctant:d03f5cc1cdb11a77410ee34e26ca1102e67a893c
//! ```
//! q process can connect to this acceptor with the acceptor's host, port and credential.
//! ```q
