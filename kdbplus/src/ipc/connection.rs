...
impl QStream{
  ...
  ///   // To disable authentication, pass `""` as credential.
  ...
  /// ```
  /// # Note
  /// - A file path of a credential file must be set on `KDBPLUS_ACCOUNT_FILE`. Credentials must be written in each line with the format
  ///  `"user:sha1_encoded_password"`. For example, user `reluctant` and password `"slowday"` is written as
  ///  `reluctant:d03f5cc1cdb11a77410ee34e26ca1102e67a893c`. In q, the encoded password can be generated by `.Q.sha1 "slowday"`.
  /// - For TLS acceptor, user needs to set a path to a pkcs12 file to an environmental variable `KDBPLUS_TLS_KEY_FILE`
  ///  and a password of the file to `KDBPLUS_TLS_KEY_FILE_SECRET`. Then a function `.kdbplus.close_tls_connection_` will be defined
  ///  on kdb+ side by asynchronous call from the server side. This function is necessary to close the socket from the
  ///  server side without crashing server side application.
  /// - For Unix domain socket acceptor, user needs to set an environmental variable `QUDSPATH` to the directory where a socket file is created.
