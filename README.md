...
*Notes:*
- For acceptor, a file path of a credential file must be set on `KDBPLUS_ACCOUNT_FILE`. Credentials must be written in each line with the format `"user:sha1_encoded_password"`. For example, user `reluctant` and password `"slowday"` is written as `reluctant:d03f5cc1cdb11a77410ee34e26ca1102e67a893c`. In q, the encoded password can be generated by `.Q.sha1 "slowday"`.
- For TS client, you need to set two environmental variables `KX_SSL_CERT_FILE` and `KX_SSL_KEY_FILE` to make kdb+ to work as a TLS server. For details, see [the KX website](https://code.kx.com/q/kb/ssl/).
